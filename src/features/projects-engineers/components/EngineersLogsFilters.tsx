...
interface Props {
  filters: GetEmploersActionsLogsFilters;
  onChange: (next: GetEmploersActionsLogsFilters) => void;
}
...
const EngineersLogsFilters = ({ filters, onChange }: Props) => {
  ...
  return (
    <SidebarFilters reset={reset} activeCount={activeCount}>
      <div className="mt-4 space-y-6">
        <Input
          label={t("projectsEngineers.logs.search.label", "Search")}
          ...
          placeholder={t(
            "projectsEngineers.logs.search.placeholder",
            "Search by engineer or activity...",
          )}
        />
        ...
        <div className="space-y-2">
          <Label className="text-xs uppercase tracking-wider text-muted-foreground">
            {t("projectsEngineers.logs.filters.activity", "Activity Type")}
          </Label>
          <div className="space-y-2">
            {ENGINEERS_PROJECT_ACTIONS.map((action) => {
              const checked = filters.action === action;
              return (
                <label
                  key={action}
                  onClick={(e) => {
                    ...
                    onChange({
                      ...filters,
                      action: checked ? undefined : action,
                    });
                  }}
...
export default EngineersLogsFilters;
