  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        aria-expanded={open}
      >
        <img src={current.icon} alt={current.code} className="h-5 w-5 rounded-sm" />
        <span className="hidden sm:inline">{current.label}</span>
      </button>

      {open && (
        <ul className="absolute right-0 z-50 mt-2 w-36 rounded-lg border border-slate-200 bg-white py-1 shadow-lg">
          {languages.map((lang) => (
            <li
              key={lang.code}
            ...
            >
              <img src={lang.icon} alt={lang.code} className="h-4 w-4 rounded-sm" />
              <span className="flex-1">{lang.label}</span>
              {language === lang.code && <span className="text-xs text-slate-400">✓</span>}
            </li>
          ))}
        </ul>
      )}
